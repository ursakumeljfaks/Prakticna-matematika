...
class Drevo:
    ...
    def koliko_ponikne(self):
        if self.prazno:
            return 0
        koliko = max(0, self.levo.get_vrednost() + self.desno.get_vrednost() - self.vrednost)
        koliko += self.levo.koliko_ponikne()
        koliko += self.desno.koliko_ponikne()
        return koliko
...
class Drevo(Drevo):
    def goljufi(self):
        nabor = set()
        if not self.prazno:
            koliko = self.levo.get_vrednost() + self.desno.get_vrednost() - self.vrednost
            if koliko > 0:
                nabor.add((self.ime, koliko))
            nabor.update(self.desno.goljufi())
            nabor.update(self.levo.goljufi())
        return nabor
...
class Drevo(Drevo):
    def zasluzek(self):
        if self.prazno:
            return (0,0)
        if self.desno.prazno and self.levo.prazno:
            return (self.vrednost,0)
        else:
            veliki = 0
            if not self.levo.prazno:
                veliki += self.levo.zasluzek()[0] + self.levo.zasluzek()[1]
            if not self.desno.prazno: 
                veliki += self.desno.zasluzek()[0] + self.desno.zasluzek()[1]

            mali = self.desno.zasluzek()[1] + self.levo.zasluzek()[1]
            if self.levo.prazno or self.desno.prazno:
                mali += self.vrednost
            return (veliki, mali)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ1MywidXNlciI6NjQ4M30:1rBbvO:Qo6-Qr83FVOpab_s8qM2bWjNsH_rApkZNh_gfV4nsR0"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ1NCwidXNlciI6NjQ4M30:1rBbvO:zSQmUV6MNskvfJGY1_JbOWElLTu1goRqbLpLRAwVPYU"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ1NSwidXNlciI6NjQ4M30:1rBbvO:WUKoVSPi_cpq6LI3z08VltHii6MW83E7EYY1W38Ae4U"
        try:
            test_data = [
