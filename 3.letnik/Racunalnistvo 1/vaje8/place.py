...
class Drevo:
    ...
    def odprava_krivic(self):
        ...
        if not self.levo.prazno:
            razlika_l = max(0, self.levo.placa + 100 - self.placa)
            koliko += razlika_l + self.levo.odprava_krivic()
        if not self.desno.prazno:
            razlika_d = max(0, self.desno.placa + 100 - self.placa)
            koliko += razlika_d + self.desno.odprava_krivic() 
        return koliko 
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ2NywidXNlciI6NjQ4M30:1rESKN:w-nMd0fJhATz8vBDSwXqGNrfVTli9NJ-QRS59CnfYsI"
        try:
            lucka = Drevo('Lučka', 800, levo=Drevo('Peter', 900), desno=Drevo('Tadeja', 700))
            matjaz = Drevo('Matjaž', 1100, levo=Drevo('Simona', 700), desno=Drevo('Boris', 1000, levo=lucka))
            branko = Drevo('Branko', 900, desno=Drevo('Benjamin', 1100))
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ2OCwidXNlciI6NjQ4M30:1rESKN:rUFeXizoIQqJViBHRJ7ig_vf7FapLqqa-BWEYX0u6uw"
        try:
            lucka = Drevo('Lučka', 800, levo=Drevo('Peter', 900), desno=Drevo('Tadeja', 700))
            matjaz = Drevo('Matjaž', 1100, levo=Drevo('Simona', 700), desno=Drevo('Boris', 1000, levo=lucka))
            branko = Drevo('Branko', 900, desno=Drevo('Benjamin', 1100))
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ2OSwidXNlciI6NjQ4M30:1rESKN:gIba5_CokYc0IFz7tkSnNT3oAAS_mmz8p6jKy9DFCiM"
        try:
            lucka = Drevo('Lučka', 800, levo=Drevo('Peter', 900), desno=Drevo('Tadeja', 700))
            matjaz = Drevo('Matjaž', 1100, levo=Drevo('Simona', 700), desno=Drevo('Boris', 1000, levo=lucka))
            branko = Drevo('Branko', 900, desno=Drevo('Benjamin', 1100))
