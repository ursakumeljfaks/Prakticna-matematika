...
def drevo_vmesni_premi(vmesni, premi):
    ...
    levo_d = drevo_vmesni_premi(vmesni[:i], premi[1:i+1])
    desno_d = drevo_vmesni_premi(vmesni[i+1:], premi[i+1:])
    return Drevo(koren, levo=levo_d, desno=desno_d)
# =====================================================================@038477=
# 2. podnaloga
...
def drevo_vmesni_obratni(vmesni, obratni):
    ...
    koren = obratni[-1]
    i = vmesni.index(koren)
    levo_d = drevo_vmesni_obratni(vmesni[:i], obratni[:i])
    desno_d = drevo_vmesni_obratni(vmesni[i+1:], obratni[i:-1])
    return Drevo(koren, levo=levo_d, desno=desno_d)
# =====================================================================@038478=
# 3. podnaloga
...
def drevo_vmesni_nivojski(vmesni, nivojski):
    ...
    levi_d = []
    desni_d = []
    for crka in nivojski[1:]:
        if crka in levi:
            levi_d.append(crka)
        else:
            desni_d.append(crka)
    levo_d = drevo_vmesni_nivojski(levi, levi_d)
    desno_d = drevo_vmesni_nivojski(desni, desni_d)
    return Drevo(koren, levo=levo_d, desno=desno_d)
...
def drevesa_premi_obratni(premi, obratni):
    ...
    if len(premi) > 2:
        ...
        for levo in vsa_leva:
            for desno in vsa_desna:
                drevesa.add(Drevo(koren, levo=levo, desno=desno))
        return drevesa
    if len(premi) == 2:
...
def drevesa_vmesni_premi(vmesni, premi):
    ...
    koren = premi[0]
    drevesa = set()
    indeksi = [i for i in range(len(vmesni)) if vmesni[i] == koren]
    for i in indeksi:
        levo_d = drevesa_vmesni_premi(vmesni[:i], premi[1:1+i])
        desno_d = drevesa_vmesni_premi(vmesni[i+1:], premi[1+i:])
        for levo in levo_d:
            for desno in desno_d:
                drevesa.add(Drevo(koren, levo=levo, desno=desno))
    return drevesa    
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ3NiwidXNlciI6NjQ4M30:1rE7Ae:ENcVhYLOehyjg8VIWrznVTmhlrAl1c7zXzwQE3vHdfk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ3NywidXNlciI6NjQ4M30:1rE7Ae:zjBGB5IoqQ9bS3aDgkKgtrcDKbAkf-p_CZkX0X1sRRk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ3OCwidXNlciI6NjQ4M30:1rE7Ae:rfPLyBeS71PP-xwUguO5jl47gvd3hNjgfz_2t4UIWak"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ3OSwidXNlciI6NjQ4M30:1rE7Ae:spNZXFikdNj6uV8SeS39VyKb3j_kwhnETluNWf5YTKI"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODQ4MCwidXNlciI6NjQ4M30:1rE7Ae:cC0yobLkfx1BpAjmyeNxWEd8rT-jz0KsSLo6tDp2Ewk"
        try:
