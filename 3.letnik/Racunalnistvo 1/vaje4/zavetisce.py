...
class Zavetisce:
    ...
    def sprejmi(self, ime, vrsta):
        if vrsta == 'pes':  
            ...
            self.macke.vstavi(ime)
        else:
            raise Exception
    
    def oddaj_psa(self):
        if not self.psi.prazna():
            oddaja = self.psi.zacetek()[0]
            self.psi.odstrani()
            return oddaja
        else:
            return None
    ...
    def oddaj_zival(self):
        if not self.psi.prazna() and not self.macke.prazne():
            if self.psi.zacetek() < self.macke.zacetek():
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM0OSwidXNlciI6NjQ4M30:1qyvj8:hGl36RlJNV3tvrTWJlAeFlJi0yWe_EyBtaLL8z3Cy5c"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozODM1MCwidXNlciI6NjQ4M30:1qyvj8:UWUIT5PlxtI4-87c7ab5ffQ8h7eXF_l3WY3q2CDj_kE"
        try:
