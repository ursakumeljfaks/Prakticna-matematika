...
def skupna_dolzina(ttn):
    """vrne skupno dolzino vseh nizov v tabeli tabel ttn"""
    dolzina = 0
    for niz in ttn:
        if isinstance(niz, str): #preveri ali je niz zares niz in vrne True
            dolzina += len(niz) #ce je potem je dolzina enaka vsoti vseh znakov
        else:
            dolzina += skupna_dolzina(niz) #drugace gre se enkrat skozi zanko in gleda naprej
    return dolzina
# =====================================================================@028341=
# 2. podnaloga
...
# =============================================================================
def max_dolzina(ttn):
    """vrne dolzino najdaljsega niza v tabeli nizov ttn"""
    najdaljsi = 0
    for niz in ttn:
        if isinstance(niz, list):
            trenutni_naj = max_dolzina(niz)
            if trenutni_naj > najdaljsi:
                najdaljsi = trenutni_naj
        else:
            trenutni_naj = len(niz)
            if trenutni_naj > najdaljsi:
                najdaljsi = trenutni_naj
    return najdaljsi
...
#     'oblak'
# =============================================================================
def max_dolzina_zadnji(ttn):
    """vrne najdaljsi niz v tabeli nizov ttn, ce je takih nizov vec vrne zadnjega"""
    # for niz in ttn:
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM0MCwidXNlciI6NjQ4M30:1nNHCU:RsWbX8cwkK0csCdPnuUSD7AlkeQ'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM0MSwidXNlciI6NjQ4M30:1nNHCU:QdbnW321Pwd5S7Y-CsaobYvJKJ4'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM0MiwidXNlciI6NjQ4M30:1nNHCU:7KQRIoEt916HICgg2n7Cf-sMqGg'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM0MywidXNlciI6NjQ4M30:1nNHCU:0ypQ7GKXAD2QArxth0GPEKomr4Y'
        try:
