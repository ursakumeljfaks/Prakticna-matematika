...
# =====================================================================@028327=
# 1. podnaloga
# Iz vesolja pričakuješ signal nezemeljske civilizacije. Utemeljeno lahko 
# pričakuješ, da bo sporočilo poslano v jeziku, ki uporablja angleško abecedo 
# 26 malih črk in zadošča naslednjim pravilom:
# 
#  * Sporočilo je sestavljeno zgolj iz zaporedja znakov a, b, c, d, . . . , z,
#    ki sestavljajo besede, in iz presledkov med njimi (brez ločil).
...
# 
# #### Naloga
# Preuredi vrstice funkcije `pravo(niz)` tako, da bo ugotovila, ali sporočilo 
# ustreza tem pogojem.
# 
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODMyN30:1n1ny7:OmVc0-b2R8f_zR8lvk14fMu2xNs'
        try:
            Check.equal("pravo('tell us about you')", False)
            Check.equal("pravo('exchange information')", False)
            Check.equal("pravo('we live in same universe')", True)
            Check.equal("pravo('aloha')", True)
            Check.secret(pravo('aaaa'))
            Check.secret(pravo('we are smart'))
        except:
