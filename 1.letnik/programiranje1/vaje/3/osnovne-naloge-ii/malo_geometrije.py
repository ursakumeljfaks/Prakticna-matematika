...
#       from math import *
# 
# To je malo drugačen način dostopa do knjižnice, ki omogoča,
# da funkcije iz knjižnice math kličemo brez tega, da spredaj napišemo `math.`
# 
# pri teh podnalogah morate obvezno uporabljati `from math import *`
# =====================================================================@028085=
# 1. podnaloga
...
# Konstanta $\pi$ se nahaja v spremenljivki z imenom `pi` potem ko smo uvozili knjižnico `math`,
...
obseg = round(2 * pi * polmer, 2)
ploscina = round(pi * polmer ** 2, 2)
print("Obseg kroga:", obseg, "\nPovršina kroga:", ploscina)
# =====================================================================@028086=
# 2. podnaloga
...
s = (a + b + c) / 2
ploscina = round(sqrt(s * (s - a) * (s - b) * (s - c)), 2)
...
ploscina = round(sqrt(s * (s - a) * (s - b) * (s - c)), 2)
r = ploscina / s
ploscina_vcrtanega_kroga = round(pi * (r ** 2), 2)
...
r = (a * b * c) / (4 * ploscina)
ploscina_ocrtanega_kroga = round(pi * (r ** 2), 2)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODA4NX0:1mxbsY:m2V4Qh3Vp4JnpJK2pYjImQrde2E'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODA4Nn0:1mxbsY:yQi4un066ynD4Sd-OBaOCXhJas8'
        try:
            ...
            for i in range(len(vhod)):
                with Check.input([*vhod[i]]):
                    a, b = vhod[i]
                    c = str(sqrt(float(a)**2 + float(b)**2))
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODA4N30:1mxbsY:X_ToJ2M3tzS-IMGcYYVgLxXqqlk'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODA4OH0:1mxbsY:CFTuae5ujeQHZT4rED67bUmwbhc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODA4OX0:1mxbsY:L5oMWi85qUjH-XcPHSAt2yyAMtY'
        try:
