...
def izpisi_datoteke(pot_do_mape):
    """izpise vse datoteke v izbrani mapi in njenih podmapah"""
    for dat in os.listdir(pot_do_mape):
        if os.path.isdir(pot_do_mape + "\n" + dat):
            print(f"{dat}")
...
def izpisi_py(pot_do_mape):
    """poisce imena vseh datotek v mapi, ki imajo koncnico .py"""
    imena = ""
    for dat in os.listdir(pot_do_mape):
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDE5OSwidXNlciI6NjQ4M30:1niDJy:SX1UTlYcp_9JM90uI5jrsfhXoMY'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDIwMCwidXNlciI6NjQ4M30:1niDJy:DlJ2BQ4GFrMkhTs23hX3RQ9C0SY'
        try:
