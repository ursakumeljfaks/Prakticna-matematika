...
def seznam_vseh_podmap(zacetna_mapa):
    """vrne seznam poti do vseh map, ki vsebuje zacetno mapo, ce te ni vrne None"""
    if not os.path.isdir(zacetna_mapa):
        return None
    # zacetna mapa je imenik
    vsi_imeniki = [zacetna_mapa]
    # ali je v tem imeniku se kaj imenikov
    for dat in os.listdir(zacetna_mapa):
        # ali je dat imenik?
        prava_pot = zacetna_mapa + "\\" + dat
        if os.path.isdir(prava_pot):
            vsi_imeniki.extend(seznam_vseh_podmap(prava_pot))
    return vsi_imeniki
# =====================================================================@030202=
# 2. podnaloga
...
def mape_ki_vsebujejo_datoteko(zacetna_mapa, datoteka):
    """vrne urejen seznam vseh map, ki vsebujejo dano datoteko"""
    if not os.path.isdir(zacetna_mapa):
        return None

    vsi_imeniki = []
    # gremo cez zacetna_mapa
    for dat in os.listdir(zacetna_mapa):
        # ce je datoteka, po kateri gremo enaka iskani, jo dodamo v vsi_imeniki
        if dat == datoteka:
            vsi_imeniki.append(zacetna_mapa)
        prava_pot = zacetna_mapa + "\\" + dat
        if os.path.isdir(prava_pot):
            vsi_imeniki.extend(mape_ki_vsebujejo_datoteko(prava_pot, datoteka)) 
    vsi_imeniki.sort()
    return vsi_imeniki
# =====================================================================@030203=
# 3. podnaloga
...
# šteje. Če podana pot ne opisuje mape, naj funkcija vrne `None`.
# 
#     >>>> koliko_datotek('.\\PythonOSRek\\test1')
#     46
# =============================================================================
def koliko_datotek(zacetna_mapa):
    """presteje, koliko je vseh datotek v izbrani mapi, naj steje tudi datoteke v podmapah, map pa naj ne steje"""
    if not os.path.isdir(zacetna_mapa):
        return None
    
    stevilo_vseh = 0
    for dat in os.listdir(zacetna_mapa):
        if os.path.isfile(zacetna_mapa + "\\" + dat):
             stevilo_vseh += 1
        elif os.path.isdir(zacetna_mapa + "\\" + dat):
            stevilo_vseh += koliko_datotek(zacetna_mapa + "\\" + dat)
    return stevilo_vseh
# =====================================================================@030204=
# 4. podnaloga
...
def vrni_python_datoteke(zacetna_mapa):
    """vrne po abecedi urejen seznam imen vseh datotek v izbrani mapi in njenih podmapah, ki imajo .py koncnico"""
    if not os.path.isdir(zacetna_mapa):
        return None
    ...
    for dat in os.listdir(zacetna_mapa):
        if os.path.isfile(zacetna_mapa + "\\" + dat):
            if os.path.splitext(dat)[1] == ".py":
                seznam_dat.append(zacetna_mapa + "\\" + dat)
        elif os.path.isdir(zacetna_mapa + "\\" + dat):
            seznam_dat.extend(vrni_python_datoteke(zacetna_mapa + "\\" + dat))
    return sorted(seznam_dat)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDIwMSwidXNlciI6NjQ4M30:1niDJy:nZqvfRtrwEVhWZlq1LyMqb_5ow0'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDIwMiwidXNlciI6NjQ4M30:1niDJy:HDUftsjhx1B0itLqZoA1UVPvyHo'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDIwMywidXNlciI6NjQ4M30:1niDJy:xCsBbvcvPoX1H7kian5nWXnaLr8'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDIwNCwidXNlciI6NjQ4M30:1niDJy:4WodQQWQ2CtlbChx6h_9CejblR4'
        try:
