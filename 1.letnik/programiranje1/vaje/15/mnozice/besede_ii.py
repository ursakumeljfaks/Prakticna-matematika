...
def podobna(beseda, tab_besed):
    """vrne besedo iz tab_besed, ki ima najvec ujemajocih crk z besedo beseda"""
    beseda = beseda.lower()
    beseda = set(beseda)
    if len(tab_besed) == 0:
        return None
    najvecja = tab_besed[0]
    velikost_preseka = len(set(najvecja.lower()) & beseda)
    for bes in tab_besed:
        najvecja_male = najvecja.lower()
        bes_male = bes.lower()
        mnozica_bes = set(bes_male)
        if len(mnozica_bes & beseda) > len(set(najvecja_male) & beseda):
            najvecja = bes
            velikost_preseka = len(mnozica_bes & beseda)
    if velikost_preseka > 0:
        return najvecja
    else:
        return None
# =====================================================================@028389=
...
def razlicni_znaki(tab_besed):
    """vrne urejeno tabelo, ki imajo vse znake razlicne"""
    tab_besed.sort()
    pregledane_besede = set()
    nova_tabela = []
    for beseda in tab_besed:
        v_seznamu = True
        if beseda not in pregledane_besede:
            for crka in beseda:
                if beseda.count(crka) != 1:
                    v_seznamu = False
            if v_seznamu:
                nova_tabela.append(beseda)
            pregledane_besede.add(beseda)
    return nova_tabela
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM4OCwidXNlciI6NjQ4M30:1nKkXZ:nTa4aSygFdZjwV2R-5TZJPk1_2k'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM4OSwidXNlciI6NjQ4M30:1nKkXZ:Yg47HNiEHHPQZIrikPMXCur98Ps'
        try:
