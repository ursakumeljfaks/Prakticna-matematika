...
def naj_raznolika_ang_beseda():
    """vrne po abecedi urejeno tabelo vseh tistih besed, ki imajo največ različnih črk"""
    import urllib.request
    naslov = "http://splasho.com/upgoer5/phpspellcheck"
    naslov += "/dictionaries/1000.dicin"
    vir = urllib.request.urlopen(naslov)
    for vr in vir:
        ...
        nova_tabela = []
        najvecji = vrstica
        najvecji = len(set(vrstica.lower()))
        st_crk = len(set(vrstica.lower()))
        if st_crk > najvecji:
            najvecji = st_crk
            nova_tabela.append(vrstica)
        elif st_crk == najvecji:
            najvecji = st_crk
            nova_tabela.append(vrstica)
    return nova_tabela
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM5MCwidXNlciI6NjQ4M30:1nKkXZ:BDdBsoTj7ewWa3WzQ4QGMGQqG2I'
        try:
