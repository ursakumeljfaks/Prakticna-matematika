...
# =============================================================================
def narcisoidi(d):
    """vrne mnozico tistih, ki ljubjo same sebe"""
    narcis = set()
    for ime, zaljubljen in d.items():
        if ime in zaljubljen:
            narcis.add(ime)
    return narcis
# =====================================================================@028405=
# 2. podnaloga
...
# =============================================================================
def ljubljeni(d):
    """vrne mnozico tistih, ki so ljubljeni"""
    ljubljeni = []
    for oseba in d:
        for ljubljen in d[oseba]:
            if ljubljen not in ljubljeni:
                ljubljeni.append(ljubljen)
    return sorted(ljubljeni)
# =====================================================================@028406=
# 3. podnaloga
...
def pari(d):
    ...
    zaljubljeni = set()
    for oseba in d:
        for zaljubljen in d[oseba]:
            if oseba in d[zaljubljen] and oseba not in zaljubljen:
                par = tuple(sorted([oseba, zaljubljen]))
                zaljubljeni.add(par)
    return zaljubljeni   
...
def zaljubljeni_v_osebo(oseba, slovar):
    """vrne mnozico vseh ljudi, ki so do dane osebe se posebej ustreljivi"""
    ustrezljivi = set()
    for ustrezljiva, zaljubljeni in slovar.items():
        if oseba in zaljubljeni:
            ustrezljivi.add(ustrezljiva)
    return ustrezljivi
# =====================================================================@028407=
# 5. podnaloga
...
def ustrezljivi(oseba, slovar):
    ...
    while len(dodani) > 0:
        ...
        for dodan in dodani:
            zacasna.update(zaljubljeni_v_osebo(dodan, slovar))
        dodani = zacasna - ustrezljivi
    return ustrezljivi
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODQwNCwidXNlciI6NjQ4M30:1nKkYH:xxSmcV0uotc9eWudDi-GB5bnJfA'
        try:
            sola = {'Ana': {'Bine', 'Cene'},
                      'Bine': set(),
                      'Cene': {'Bine'},
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODQwNSwidXNlciI6NjQ4M30:1nKkYH:Pp0mN-U8v8ZUhjEyCKQIjsJhzig'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODQwNiwidXNlciI6NjQ4M30:1nKkYH:A7XYopnOK9XSlhWO6VHhpPknFs0'
        try:
            sola2 = {'Ana': {'Bine', 'Cene'},
                       'Bine': {'Ana'},
                       'Cene': {'Bine'},
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDA1NCwidXNlciI6NjQ4M30:1nKkYH:DZqw8htq2_-O4bSy2XBRakyEVHw'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODQwNywidXNlciI6NjQ4M30:1nKkYH:xpaSGicxvH_JDuGYQSFCIBdsCwE'
        try:
