...
# 
# Sestavi funkcijo `ploscina_val(n)`, ki izračuna ploščino vala funkcije
# `sin(x)` po opisani metodi. Število naključnih točk
# funkcija dobi kot parameter.
# =============================================================================
import random
import math
def je_pod_funkcijo(x,y):
    """vrne true, ce je tocka (x,y) pod sin(x)"""
    return y <= math.sin(x)
    
def ploscina_val(n):
    """izracuna ploscino vala funkcije sin(x) po opisani metodi"""
    stevec = 0
    for _ in range(n):
        x = random.uniform(0, math.pi)
        y = random.random()
        if je_pod_funkcijo(x,y):
            stevec += 1
    return math.pi * stevec / n
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODI2NH0:1n7FLw:43xNfnVFDlaxVVTRtPIcZp0LJwk'
        try:
            random.seed(42)
            Check.equal('ploscina_val(100)', 2.0106192982974678) and \
            Check.equal('ploscina_val(1000)', 2.0420352248333655) and \
            Check.equal('ploscina_val(10000)', 1.9955396535602365) and \
            Check.equal('ploscina_val(100000)', 1.9983042550953956)
            # Check.equal('ploscina_val(1000000)', 2.001411290229796)
            # Check.equal('ploscina_val(2000000)', 1.9994446532286487)
            # Check.equal('ploscina_val(1000000)', 2.0001420867977457)
        except:
