...
def pomnozi(recept, faktor):
    """vrne nov recept, pomnozen s faktorjem"""
    nov_recept = {}
...
def imamo_sestavine(recept, shramba):
    """ali ima v shrambi dovolj sestavin"""
    for sestavina in recept:
...
def potrebno_kupiti(recept, shramba):
    """vrne slovar sestavin, ki jih mora Metka se kupiti"""
    potrebuje = dict()
    for sestavina in recept:
        if recept[sestavina] > shramba.get(sestavina, 0):
            potrebuje[sestavina] = recept[sestavina] - shramba.get(sestavina, 0)
    return potrebuje
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODM4Mn0:1n7FKI:OHGrskAYHDKWXX_T_pdwCkfiM1Q'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODM4M30:1n7FKI:UWqTgF3WBmJkxwsNqWJ4uL_eydI'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODM4NH0:1n7FKI:O0rfXBp4ZTutfxpdfSLjrqnB6Kw'
        try:
