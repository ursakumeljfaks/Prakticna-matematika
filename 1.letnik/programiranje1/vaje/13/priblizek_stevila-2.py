...
# 
# Sestavi funkcijo `priblizek_pi(st_tock)`, ki izračuna približek števila π po
...
import random
def je_v_krogu(x,y):
    """vrne true, ce je tocka (x,y) v cetrtini kroga"""
    razdalja = x ** 2 + y ** 2
    return razdalja <= 1

def priblizek_pi(st_tock):
    """izracuna priblizek stevila pi"""
    stevec = 0
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODI2M30:1n7FKO:Wh_hNhVHVJY0Txybl5SXcGVNgEw'
        try:
            import random
            ...
            random.seed(11)
            Check.equal("priblizek_pi(10000)", 3.1376) and \
            Check.equal("priblizek_pi(100)", 3.12) and \
            Check.equal("priblizek_pi(100000)", 3.14848)
        except:
