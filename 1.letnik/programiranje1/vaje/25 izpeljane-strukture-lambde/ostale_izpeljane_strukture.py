...
def mnozica_sodih(stevila):
    """vrne mnozico sodih elementov seznama stevila"""
...
def ustvari_slovar(seznam_parov):
    """vrne slovar. ki ima za kljuce k_i, za vrednosti pa v_i"""
    return {k: v for k,v in seznam_parov}
# =====================================================================@030234=
# 4. podnaloga
...
def obrni_slovar(slovar):
    """vrne nov seznam, ki ima za kljuce vrednosti starega slovarja, za vrednosti pa kljuce starega slovarja"""
...
def pomnozi(slovar, k):
    """vrne nov slovar, kjer so vrednosti slovarja pomnožene s k"""
    return {kl: v * k for kl, v in sloavr.items()}
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDIzMSwidXNlciI6NjQ4M30:1nk1Ez:ctXleThWqPAmF_vuic47yXNWMgc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDIzMiwidXNlciI6NjQ4M30:1nk1Ez:lmsrRu1cq8uZkYPHfKaK58hv50k'
        try:
            s = Check.current_part['solution']
            true_lines = [line for line in s.split('\n') if not any(line.strip().startswith(char) for char in ['#', '\'', '\"', 'def'])]
            if len(true_lines) > 1:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDIzMywidXNlciI6NjQ4M30:1nk1Ez:1B0Y5_c_eS6HQqgKSHzC6Hhhbl4'
        try:
            s = Check.current_part['solution']
            true_lines = [line for line in s.split('\n') if not any(line.strip().startswith(char) for char in ['#', '\'', '\"', 'def'])]
            if len(true_lines) > 1:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDIzNCwidXNlciI6NjQ4M30:1nk1Ez:-c6rSDyZYYBhji738LQkMxIeAYM'
        try:
            s = Check.current_part['solution']
            true_lines = [line for line in s.split('\n') if not any(line.strip().startswith(char) for char in ['#', '\'', '\"', 'def'])]
            if len(true_lines) > 1:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDIzNSwidXNlciI6NjQ4M30:1nk1Ez:fG8nEkSvbfYH1RCOkAz_aqsMepg'
        try:
            s = Check.current_part['solution']
            true_lines = [line for line in s.split('\n') if not any(line.strip().startswith(char) for char in ['#', '\'', '\"', 'def'])]
            if len(true_lines) > 1:
