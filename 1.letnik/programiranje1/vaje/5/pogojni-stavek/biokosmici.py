...
def nakup(imam, rabim, vrecka):
    """vrne število vreèk, ki jih potrebujemo"""
    se = (rabim - imam) / vrecka
    dec = se - int(se)
    if dec != 0:
        return int(se) + 1
    elif imam > rabim:
...
def napolni(kosmici, rozine, orescki, n):
    """Funkcija vrne koliko posameznih kolicin moramo se kupiti, da bomo napolnili n vreck."""
    mora_kosmici = n * 80
    mora_rozine = n * 20
    mora_orescki = n * 25
    if mora_kosmici - kosmici < 0:
        return 0, mora_rozine - rozine, mora_orescki - orescki
    elif mora_rozine - rozine < 0:
        return mora_kosmici - kosmici, 0, mora_orescki - orescki
    elif mora_orescki - orescki < 0:
        return mora_kosmici - kosmici, mora_rozine - rozine, 0
    return mora_kosmici - kosmici, mora_rozine - rozine, mora_orescki - orescki
# =====================================================================@028144=
# 3. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODE0Mn0:1mxbth:ok5S3Y4vYGWe07PMfQmXMv8Tuqc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODE0M30:1mxbth:v0oBzOrzE-b5Ff34qF3G27OeTJA'
        try:
            s = Check.current_part['solution']
            sw = re.sub('[ \n]', '', s)
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo2NDgzLCJwYXJ0IjoyODE0NH0:1mxbth:JQjenSsDBCmNwOWV-9o_2mKGonQ'
        try:
            s = Check.current_part['solution']
            sw = re.sub('[ \n]', '', s)
