...
def odstrani_prazne_vrstice(vhodna, izhodna):
    """vse kitice bo zdruzila in jih zapisala v izhodno datoteko"""
    with open(vhodna) as vhodna_dat:
        with open(izhodna, "w") as izhodna_dat:
            for vrstica in vhodna_dat:
                if vrstica != "\n":
                    izhodna_dat.write(vrstica)
    return izhodna_dat
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODQ0MiwidXNlciI6NjQ4M30:1nU4S0:8fRpurJSAb1KRFL34lEdx-74M5M'
        try:
