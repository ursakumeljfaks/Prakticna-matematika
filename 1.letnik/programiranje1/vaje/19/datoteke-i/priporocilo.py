...
def izpisi_drugo(vhodna, izhodna):
    ...
    upostevamo = True
    with open(izhodna,"w") as dat1:
        with open(vhodna, "r") as dat2:
            for vrstica in dat2:
                if upostevamo:
                    vrstica = vrstica.strip()
                    print(vrstica, file=dat1)
                    upostevamo = False
                else:
                    upostevamo = True
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDE1MCwidXNlciI6NjQ4M30:1nU4S0:NDsYAhmO27u7GclBAwsA7ksEhd0'
        try:
