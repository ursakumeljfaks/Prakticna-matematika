...
def prestej_vrstice(datoteka):
    """presteje, koliko vrstic je na dani znakovni datoteki"""
    koliko = 0
...
def vrni_k_vrstico(datoteka, k):
    """vrne k-to vrstico dane znakovne datoteke"""
    katera = 1 #vrstice se zacnejo steti pri 1
    with open(datoteka) as dat:
        for vrstica in dat:
            if katera == k:
                return vrstica
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDE0MywidXNlciI6NjQ4M30:1nU3uH:qenD-IZzUn2EIob4VQS72JRKReU'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDE0NCwidXNlciI6NjQ4M30:1nU3uH:RjT6hhTjbuxadQabEgw6RfShJaY'
        try:
