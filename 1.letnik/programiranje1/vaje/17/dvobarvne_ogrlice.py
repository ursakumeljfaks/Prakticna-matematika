...
def je_ogrlica(niz, b, r):
    """preveri ali je niz iz belih in rdecih kroglic"""
    bele = 0
    rdece = 0
    for kroglica in niz:
        if kroglica == "B":
...
def stevilo_ogrlic(b, r):
    """izracuna stevilo vseh oznih kroglic"""
    if b == 0 or r == 0:
        return 1
    else:
...
# =============================================================================
def tabela_kombinacij(b, r):
    tabela = []
    tabela2 = []
    while b != 0:
        tabela.append((b, r, stevilo_ogrlic(b-1, r)))
        b -= 1
    while r != 0:
        tabela2.append((b, r, stevilo_ogrlic(b, r-1)))
        r -= 1
    return tabela + tabela2

def ogrlice(b, r):
    """generira po abecedi urejene vse nize, ki predstavljajo ogrlico"""
    tabela_komb = tabela_kombinacij(b, r)
    tabela3 = []
    for tripl in tabela_komb:
        if tripl[0] == 0:
            tabela3.append(tripl[1] * "R")
        if tripl[1] == 0:
            tabela3.append(tripl[0] * "B")
        else:
            tabela3.append(tripl[0] * "B" + tripl[1] * "R")
    return tabela3
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM1MywidXNlciI6NjQ4M30:1nPmDD:XB9kiSOEqM9hZigYh3HEpwKka_c'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM1NCwidXNlciI6NjQ4M30:1nPmDD:VfY9ell2qGAX6cCKmFfYMO-BlFk'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM1NSwidXNlciI6NjQ4M30:1nPmDD:tal67B7qM5tu6RXx22OBIgqGbKM'
        try:
