...
def max_presledki_prvi(ttn):
    """vrne niz v ttn, ki vsebuje najvec presledkov"""
    presledki = 0
    tab_presledkov = []
    for niz in ttn:
        if isinstance(niz, str):
            presledki = niz.count(" ")
            if presledki > kandidat_presle:
                kandidat = niz
            # tab_presledkov.append(presledki)
        else:
            presledki = max_presledki_prvi(niz)
        
            # tab_presledkov.extend(max_presledki_prvi(niz))
    return kandidat
    # najvecji = tab_presledkov[0][1]
    # for prvi, drugi in tab_presledkov:
    #     if tab_presledkov[drugi] > najvecji:
    #         najvecji = tab_presledkov[drugi]
    #         kandidat = tab_presledkov[prvi]
    # return kandidat
# =====================================================================@028357=
# 2. podnaloga
...
def max_stevk_vsi(ttn):
    """vrne po abecedi urejeno tabelo vseh nizov v ttn, ki vsebujejo najvecje stevilo stevk"""
    stevilo = 0
    stevila = "0123456789"
    seznam = []
    for niz in ttn:
        if isinstance(niz, str):
            ...
            #stevilo += max_stevk_vsi(niz)
            seznam.extend(max_stevk_vsi(niz))
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM1NiwidXNlciI6NjQ4M30:1nPmDD:ijJvwlMJoFFy51ZqOqZLZfIOE6w'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM1NywidXNlciI6NjQ4M30:1nPmDD:XZEzXbiBqu82t48eUxojzMGZuiY'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM1OCwidXNlciI6NjQ4M30:1nPmDD:Jvgn2lANgMeOfm8hrYBKbX1uXMU'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyODM1OSwidXNlciI6NjQ4M30:1nPmDD:0ZOXUF18btEEVjYcPf4AYyvmicQ'
        try:
